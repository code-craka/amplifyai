...
async function checkRateLimit(request: NextRequest, userId: string): Promise<{ allowed: boolean; retryAfter: number }> {
...
export async function updateSession(request: NextRequest) {
  ...
  if (pathname.startsWith('/api/') || pathname.includes('/functions/')) {
    const rateLimitResult = await checkRateLimit(request, user?.id || 'anonymous')
  ...
  if (user && isAuthRequiredRoute) {
    console.log(`[Security] User ${user.id} accessed ${pathname} at ${new Date().toISOString()}`)
  }
