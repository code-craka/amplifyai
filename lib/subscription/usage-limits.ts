...
export async function checkUserLimits(): Promise<{
  ...
  try {
    ...
    const { data: limitsData, error: limitsError } = await supabase
      .rpc('check_usage_limits', { user_uuid: user.id })
    ...
    const { data: subData, error: subError } = await supabase
      .from('subscriptions')
      .select('plan_type, status, monthly_post_limit, brand_limit, team_member_limit')
      .eq('user_id', user.id)
      .single()
    ...
    return { limits: null, subscription: null, error: 'Failed to check limits' }
  }
}
...
export async function enforceUsageLimit(action: 'post' | 'brand'): Promise<void> {
  ...
  if (action === 'post' && !limits.can_generate_post) {
    ...
    throw new UsageLimitError(message, subscription.plan_type === 'free', 'post')
  }

  if (action === 'brand' && !limits.can_create_brand) {
    const message = formatLimitMessage('brand', limits, subscription)
    throw new UsageLimitError(message, subscription.plan_type === 'free', 'brand')
  }
}