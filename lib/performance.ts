...
export class PerformanceMonitor {
  ...
  private enabled: boolean = typeof window !== 'undefined' && 'performance' in window
...
export function measureMethod(target: { constructor: { name: string } }, propertyName: string, descriptor: PropertyDescriptor) {
...
export function usePerformanceMonitor() {
  const measureAsync = React.useCallback(async <T>(name: string, operation: () => Promise<T>): Promise<T> => {
    return perf.measureAsync(name, operation)
  }, [])

  const measureSync = React.useCallback(<T>(name: string, operation: () => T): T => {
...
export const AmplifyAIMetrics = {
  ...
  measureContentGeneration: async <T>(operation: () => Promise<T>): Promise<T> => {
    return perf.measureAsync('ai-content-generation', operation)
  },

  // Database Operations
  measureDatabaseQuery: async <T>(queryName: string, operation: () => Promise<T>): Promise<T> => {
  ...
  // API Calls
  measureApiCall: async <T>(endpoint: string, operation: () => Promise<T>): Promise<T> => {
