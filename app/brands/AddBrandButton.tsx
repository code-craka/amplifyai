          ...
          <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-md animate-in">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input required value={name} onChange={e => setName(e.target.value)} placeholder="e.g., AmplifyAI" className="w-full p-2 border rounded mt-1"/>
              </div>
              <div>
                <label className="text-sm font-medium">Brand Description</label>
                <textarea required value={description} onChange={e => setDescription(e.target.value)} placeholder="e.g., We make AI tools for marketers..." className="w-full p-2 border rounded mt-1 h-24"/>
              </div>
              <div>
                <label className="text-sm font-medium">Tone of Voice</label>
                <input required value={tone} onChange={e => setTone(e.target.value)} placeholder="e.g., Witty, professional, and slightly futuristic" className="w-full p-2 border rounded mt-1"/>
              </div>
