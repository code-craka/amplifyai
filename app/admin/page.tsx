...
export default async function AdminPage() {
  ...
  const [
    ...
    supabase.from('generated_posts').select('*', { count: 'exact', head: true }),
    supabase.from('generated_posts').select('*', { count: 'exact', head: true }).eq('status', 'scheduled'),
    supabase.from('scheduling_logs').select('*').order('processed_at', { ascending: false }).limit(10)
  ])
