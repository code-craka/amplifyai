...
export function SettingsManager({ 
  ...
  const [notificationBrowser, setNotificationBrowser] = useState(profile?.notification_browser ?? true);
  const [notificationMobile, setNotificationMobile] = useState(profile?.notification_mobile ?? false);

  // Privacy settings
  const [privacyAnalytics, setPrivacyAnalytics] = useState(profile?.privacy_analytics ?? true);
  const [privacyMarketing, setPrivacyMarketing] = useState(profile?.privacy_marketing ?? false);

  // Security settings
  const [securityTwoFactor, setSecurityTwoFactor] = useState(profile?.security_two_factor ?? false);
        ...
        <TabsContent value="privacy" className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              <Button onClick={handleSaveSecurity} disabled={loading || true} className="flex items-center gap-2">
                {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
