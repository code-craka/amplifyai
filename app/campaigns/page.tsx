...
export default async function CampaignsPage() {
  ...
  const { data: briefs } = await supabase.from('content_briefs').select('*').order('created_at', { ascending: false }).limit(5)
    ...
    <Sidebar>
      <div className="w-full animate-in flex flex-col gap-12 items-center p-8">
        <div className="w-full max-w-2xl">
          ...
          <p className="text-foreground/80 mb-6">Fill out the brief below and let the AI do the heavy lifting.</p>
        ...
        <div className="w-full max-w-4xl">
          ...
          <div className="bg-background border rounded-lg shadow-sm">
            {briefs?.map((brief: {id: string; topic: string; created_at: string; status: string}) => (
              <div key={brief.id} className="p-4 border-b flex justify-between items-center">
                <div>
                  ...
                  <p className="text-xs text-foreground/60">{new Date(brief.created_at).toLocaleString()}</p>
                </div>
                <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                    brief.status === 'completed' ? 'bg-green-100 text-green-800' :
                    brief.status === 'processing' ? 'bg-blue-100 text-blue-800' :
                    brief.status === 'error' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'
                }`}>
                  {brief.status}
                </span>
              </div>
            ...
            {(!briefs || briefs.length === 0) && (
              <p className="text-center text-foreground/60 p-8">No campaigns created yet.</p>
            )}
          </div>
        </div>
      </div>
