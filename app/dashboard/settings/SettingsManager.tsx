...
export function SettingsManager({ initialConnections, searchParams }: SettingsManagerProps) {
  // Handle toast notifications from URL parameters
  useEffect(() => {
    if (searchParams.connection && searchParams.status) {
      const connectionName = searchParams.connection.charAt(0).toUpperCase() + searchParams.connection.slice(1)
      if (searchParams.status === 'success') {
        toast.success(`${connectionName} connected successfully!`)
      } else if (searchParams.status === 'error') {
        toast.error(`${connectionName} connection failed: ${searchParams.message || 'Unknown error'}`)
      }
    }
