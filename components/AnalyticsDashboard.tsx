...
export default function AnalyticsDashboard() {
  ...
  const calculateOverallMetrics = () => {
    ...
    const totalEngagement = analytics.reduce((sum, item) => 
      sum + item.likes_count + item.shares_count + item.comments_count, 0)
    const avgEngagementRate = analytics.reduce((sum, item) => 
      sum + item.engagement_rate, 0) / analytics.length
    const avgPerformanceScore = analytics.reduce((sum, item) => 
      sum + item.performance_score, 0) / analytics.length
  ...
  const getPlatformBreakdown = () => {
    const platformStats = analytics.reduce((acc: Record<string, {
      ...
      acc[item.platform].totalEngagement += item.likes_count + item.shares_count + item.comments_count
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(metrics.avgEngagementRate * 100).toFixed(1)}%</div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{metrics.avgPerformanceScore.toFixed(0)}/100</div>
          </CardContent>
        </Card>
      </div>
            ...
            <CardContent className="space-y-4">
              {insights.length === 0 ? (
                <div className="text-center py-6 text-gray-500">
                  ...
                  <p className="text-sm">Run an analysis to get AI-powered recommendations.</p>
                </div>
              ) : (
              ...
              {topPosts.map((post, index) => (
                <div key={post.id} className="flex items-center justify-between p-3 border rounded-lg">
                  ...
                  <div className="text-right">
                    <div className="font-medium">{post.performance_score.toFixed(0)}/100</div>
                    <div className="text-sm text-gray-500">
            ...
            {Object.entries(platformStats).map(([platform, stats]) => (
              <Card key={platform}>
                <CardHeader>
                  ...
                  <CardDescription>{stats.count} posts analyzed</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <div className="font-medium">{stats.totalEngagement.toLocaleString()}</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
