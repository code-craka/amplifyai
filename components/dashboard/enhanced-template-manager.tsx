...
const mockTemplates: Template[] = [
  {
    ...
    description: "Engaging template for announcing new product launches across social platforms",
    category: "Product Marketing",
    platform: ["LinkedIn", "Twitter", "Facebook"],
    content: "🚀 Excited to announce {{product_name}}! {{description}} Available now at {{link}}",
  ...
  },
  {
    ...
    description: "Professional template for sharing industry insights and establishing thought leadership",
    category: "Thought Leadership",
    platform: ["LinkedIn"],
    content: "{{insight_hook}} Here's what I've learned about {{topic}}: {{main_points}} What's your experience with {{topic}}?",
        ...
        <CardContent className="pt-0">
          ...
          <div className="grid grid-cols-3 gap-4 text-center text-sm">
            ...
            <div className="flex items-center justify-center">
              ...
              <span className="text-muted-foreground ml-1">({template.ratings.count})</span>
            </div>
          </div>
        </CardContent>
...
export function EnhancedTemplateManager({
  ...
  const {
    ...
    async (term: string) => {
      ...
      return templates.filter(template =>
        ...
        template.tags.some(tag => tag.toLowerCase().includes(term.toLowerCase()))
      );
    },
    300
  );
  ...
  const filteredTemplates = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'performance':
          return b.performance.average_engagement - a.performance.average_engagement;
        case 'alphabetical':
    ...
    return filtered;
  }, [templates, searchResults, searchTerm, selectedCategory, selectedPlatform, showFavoritesOnly, sortBy]);

  const handleTemplateAction = useCallback((action: string, template: Template) => {
    switch (action) {
      case 'use':
  ...
  }, [onTemplateUse, onTemplateEdit, onTemplateDelete, onTemplateFavorite, onTemplateSelect]);
        ...
        return (
          <RowActions
            ...
            additionalActions={[
              ...
              {
                label: template.is_favorite ? "Remove from Favorites" : "Add to Favorites",
