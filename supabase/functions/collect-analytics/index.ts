...
const linkedinAPI: PlatformAPI = {
  platform: 'linkedin',
  fetchMetrics: async (postId: string, accessToken: string): Promise<AnalyticsData> => {
    try {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (manual_data) {
      ...
      analyticsData = await platformAPI.fetchMetrics(post_id, tokenData.access_token)
    }
...
async function generatePerformanceInsights(
  ...
  try {
    ...
    const avgEngagement = historicalData.reduce((sum, record) => 
      sum + (record.engagement_rate || 0), 0) / historicalData.length
    
    const avgPerformance = historicalData.reduce((sum, record) => 
      sum + (record.performance_score || 0), 0) / historicalData.length

    // Generate performance insights
    const currentEngagement = analyticsData.views_count ? 
      ((analyticsData.likes_count || 0) + (analyticsData.shares_count || 0) + 
       (analyticsData.comments_count || 0)) / analyticsData.views_count : 0
    ...
    if (currentEngagement > avgEngagement * 1.2) {
      insights.push({
        ...
        recommendation: 'Consider creating similar content or boosting this post to maximize reach',
      ...
      insights.push({
        ...
        recommendation: 'Review content style, timing, or hashtags for future improvements',
