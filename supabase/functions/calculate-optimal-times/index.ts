...
${JSON.stringify(topSlots.map(slot => ({
...
Keep recommendations practical and actionable.`
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a social media timing optimization expert. Analyze posting time data to provide strategic scheduling recommendations.'
          },
          {
            role: 'user',
            content: prompt
    ...
    return data.choices[0]?.message?.content || 'Unable to generate timing analysis'
...
function generateBasicTimingAnalysis(timeSlotData: TimeSlotData[], platform: string): string {
  if (timeSlotData.length === 0) {
    return 'Insufficient data for timing analysis. Continue posting to get optimal time recommendations.'
  }

  const bestSlot = timeSlotData.reduce((best, current) => 
    current.avg_engagement_rate > best.avg_engagement_rate ? current : best, timeSlotData[0])
...
function calculateConfidenceScore(postCount: number, avgEngagement: number, platformAvg: number): number {
...
function getRecommendationStrength(confidence: number): 'low' | 'moderate' | 'high' {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!analyticsData || analyticsData.length === 0) {
      return new Response(
        JSON.stringify({ 
          ...
          recommendation: 'Continue posting content to receive optimal timing insights'
        }),
        { 
          status: 404, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
    ...
    analyticsData.forEach(record => {
      ...
      slot.post_count += 1
      slot.total_engagement += (record.likes_count + record.shares_count + record.comments_count)
    ...
    timeSlots.forEach((slot, key) => {
      ...
      // Calculate averages
      slot.avg_engagement_rate = slot.performance_scores.length > 0 
        ? slot.performance_scores.reduce((sum, score) => sum + score, 0) / slot.performance_scores.length / 100
        : 0
    ...
    for (const [platformName, slots] of Object.entries(platformGroups)) {
      ...
      const platformAvgEngagement = slots.reduce((sum, slot) => 
        sum + slot.avg_engagement_rate, 0) / slots.length
    ...
    const topRecommendation = recommendations.sort((a, b) => 
      b.confidence_score - a.confidence_score)[0]
