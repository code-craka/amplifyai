...
interface ContentAnalysis {
  user_id: string
  brand_id?: string
  analysis_type: 'performance_review' | 'trend_analysis' | 'optimization_suggestions'
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert social media analyst specializing in content performance optimization. Provide actionable, data-driven insights based on engagement metrics and content patterns.'
          },
          {
            role: 'user',
            content: prompt
...
function generateAnalysisPrompt(contentData: any[], analysisType: string): string {
...
function generateBasicAnalysis(contentData: any[], analysisType: string): string {
  if (contentData.length === 0) {
    return 'Insufficient data for analysis. Continue creating content to receive insights.'
  }

  const avgEngagement = contentData.reduce((sum, item) => 
    sum + (item.engagement_rate || 0), 0) / contentData.length

  const topPerformer = contentData.reduce((best, current) => 
    (current.performance_score || 0) > (best.performance_score || 0) ? current : best, contentData[0])
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { user_id, brand_id, analysis_type, timeframe_days = 30 } = analysisRequest
...
function calculatePerformancePatterns(contentData: any[]): PerformancePattern[] {
  const patterns: PerformancePattern[] = []

  // Group by content characteristics
  const platformGroups = groupBy(contentData, 'platform')
  const hourGroups = groupBy(contentData.map(item => ({
    ...item,
  ...
  Object.entries(platformGroups).forEach(([platform, posts]) => {
    if (posts.length >= 3) {
      const avgPerformance = posts.reduce((sum, post) => 
        sum + (post.performance_score || 0), 0) / posts.length
...
function calculateTrend(posts: any[]): 'improving' | 'declining' | 'stable' {
  ...
  const firstAvg = firstHalf.reduce((sum, post) => 
    sum + (post.performance_score || 0), 0) / firstHalf.length
  const secondAvg = secondHalf.reduce((sum, post) => 
    sum + (post.performance_score || 0), 0) / secondHalf.length
...
function generateInsights(
  ...
  const bestPlatform = patterns.reduce((best, current) => 
    current.avg_performance > best.avg_performance ? current : best, patterns[0])
  ...
  const timeGroups = groupBy(contentData.map(item => ({
    ...item,
    hour: new Date(item.created_at).getHours()
  })), 'hour')
  ...
  const bestHour = Object.entries(timeGroups)
    ...
    .reduce(([bestHour, bestPosts], [hour, posts]) => {
      const avgPerformance = posts.reduce((sum, post) => 
        sum + (post.performance_score || 0), 0) / posts.length
      const bestAvg = bestPosts.reduce((sum, post) => 
        sum + (post.performance_score || 0), 0) / bestPosts.length
