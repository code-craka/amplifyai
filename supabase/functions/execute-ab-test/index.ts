...
function calculateStatisticalSignificance(
  ...
  let confidence = 0
  if (zScore >= 1.96) confidence = 0.95 // 95% confidence
  ...
  const isSignificant = confidence >= 0.80
  const winner = isSignificant ? (rateA > rateB ? 'variant_a' : 'variant_b') : null
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert A/B testing analyst specializing in social media content optimization. Provide strategic insights based on test results.'
          },
          {
            role: 'user',
            content: prompt
...
function generateBasicTestAnalysis(variantA: TestResult, variantB: TestResult): string {
  const winnerRate = Math.max(variantA.engagement_rate, variantB.engagement_rate)
  const loserRate = Math.min(variantA.engagement_rate, variantB.engagement_rate)
  const improvement = ((winnerRate - loserRate) / loserRate * 100).toFixed(1)
  
  const winner = variantA.engagement_rate > variantB.engagement_rate ? 'A' : 'B'
    ...
    if (action === 'start') {
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          completed_at: significance.isSignificant ? new Date().toISOString() : null
        })
        .eq('id', test_id)
