...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Regenerating content for post ${post_id} on ${post.platform}...`)
    ...
    const copyResponse = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-4o',
        messages: [
          { role: 'system', content: 'You are an expert social media copywriter. Create engaging, platform-optimized content that is fresh and different from the original.' },
    ...
    if (!copyResponse.ok) {
      throw new Error(`Content regeneration failed: ${copyResponse.statusText}`)
    }
