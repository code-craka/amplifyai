...
Keep analysis strategic, actionable, and focused on content differentiation.`
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert competitive intelligence analyst specializing in social media strategy. Provide strategic, actionable insights for content differentiation and competitive advantage.'
          },
          {
            role: 'user',
            content: prompt
    ...
    return {
      analysis_summary: 'Basic competitive analysis completed. Competitor shows active engagement patterns.',
      opportunities: ['Content gap analysis needed', 'Engagement timing optimization'],
...
function extractOpportunities(analysis: string): string[] {
  const opportunitiesMatch = analysis.match(/OPPORTUNITIES:(.*?)(?=THREATS:|RECOMMENDATIONS:|$)/s)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { user_id, brand_id, competitor_name, competitor_platform, competitor_handle } = competitorRequest

    if (!user_id || !competitor_name || !competitor_platform) {
      return new Response(
        JSON.stringify({ error: 'user_id, competitor_name, and competitor_platform are required' }),
        { 
          status: 400, 
    ...
    const { data: existingAnalysis } = await supabase
      ...
      .gte('last_analyzed_at', new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString()) // 7 days
      .single()
    ...
    const aiAnalysis = await analyzeCompetitorWithAI(competitorData, brandContext)
    ...
    const insights = [
      {
        ...
        recommendation: aiAnalysis.opportunities[0] || 'Monitor competitor activity regularly',
