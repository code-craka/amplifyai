...
export function useBreakpoint() {
  const [currentBreakpoint, setCurrentBreakpoint] = useState<Breakpoint | 'default'>('default');
 ...
 */
export function useResponsiveColumns(columns: BreakpointValues<number>): number {
...
export function useResponsiveSpacing() {
  ...
  const getSpacing = useCallback((mobile: number, tablet?: number, desktop?: number) => {
    if (isDesktop && desktop !== undefined) return desktop;
    if (isTablet && tablet !== undefined) return tablet;
    return mobile;
  }, [isMobile, isTablet, isDesktop]);

  const getSizing = useCallback((mobile: string, tablet?: string, desktop?: string) => {
    if (isDesktop && desktop !== undefined) return desktop;
    if (isTablet && tablet !== undefined) return tablet;
    return mobile;
  }, [isMobile, isTablet, isDesktop]);
